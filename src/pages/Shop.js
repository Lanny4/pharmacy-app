...
import { FiSearch, FiPlus, FiRefreshCw } from 'react-icons/fi'; // FiCheck видалено
  ...
  return (
    <Container className="py-5">
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        ...
        <Button variant="outline-success" onClick={() => window.location.reload()} className="rounded-pill">
          <FiRefreshCw className="me-2" /> Оновити базу
        </Button>
      </div>
          ...
          <Col key={p.id} sm={6} md={4} lg={3}>
            <Card className="h-100 border-0 shadow-sm rounded-4 overflow-hidden hover-card">
              ...
              <Card.Body className="d-flex flex-column p-4">
                ...
                <Card.Title as={Link} to={`/product/${p.id}`} className="fw-bold mb-2 fs-5 text-decoration-none text-dark">
                  {p.name}
                </Card.Title>
                ...
                <div className="mt-auto d-flex justify-content-between align-items-center">
                  ...
                  <Button
                    style={{ width: '52px', height: '52px', borderRadius: '14px', backgroundColor: '#1a4332', border: 'none' }}
                    onClick={() => handleAdd(p)}
                  >
                    <FiPlus size={24} />
                  </Button>
